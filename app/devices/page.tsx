...
export default function DevicesPage() {
  ...
  const handleClosePanel = () => {
    ...
    // Keep the device ID for a moment to avoid UI flicker
  ...
  const suspiciousDevicesCount = devices.filter(
    (d) => d.status === 'Suspicious'
  ).length;
  const criticalDevicesCount = devices.filter(
    (d) => d.status === 'Critical'
  ).length;
